		...
		if( $isArray){
			...
			foreach( $_FILES[$form_name] as $key => $all ){
				foreach( $all as $i => $val ){
					$arrayfiles[$i][$key] = $val;   
				}   
			}
			$count = count($arrayfiles);
			for($i = 0; $i<$count; $i++){
				$file = new filespec($arrayfiles[$i], $this);
				...
				// Error array filled?
				if (isset($arrayfiles[$i]['error']))
				{
					$error = $this->assign_internal_error($arrayfiles[$i][$i]['error']);
					if ($error !== false)
					{
						$file->error[] = $error;
						return $file;
				...
				if (isset($arrayfiles[$i]['size']) && $arrayfiles[$i]['size'] == 0)
				{
